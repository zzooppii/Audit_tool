...
exports.analyzeSolidityFile = analyzeSolidityFile;
...
const AST_RULES = [
    {
        ...
        severity: 'critical',
        check: (node) => node.type === 'MemberAccess' &&
    ...
    },
    {
        ...
        severity: 'high',
        check: (node) => node.type === 'MemberAccess' && node.memberName === 'delegatecall',
    },
    {
        ...
        severity: 'high',
        check: (node) => node.type === 'FunctionCall' &&
    ...
    },
    {
        ...
        severity: 'medium',
        check: (node) => node.type === 'FunctionCall' && node.expression?.name === 'blockhash',
    },
    {
...
function resolveRule(rule, config) {
...
function resolveFunctionRule(rule, config) {
    const override = config?.rules?.[rule.id];
    if (override?.enabled === false) {
        return { enabled: false, severity: rule.severity };
    }
...
function isExternalCallNode(node) {
    return (node?.type === 'FunctionCall' &&
        node.expression?.type === 'MemberAccess' &&
        ['call', 'callcode', 'delegatecall', 'staticcall', 'send', 'transfer'].includes(node.expression.memberName));
}
function isStateVarIdentifier(node, stateVars) {
    if (!node)
        return false;
...
function statementHasExternalCall(statement) {
    if (!statement)
        return false;
    let found = false;
    visit(statement, {
...
function statementHasStateWrite(statement, stateVars) {
    if (!statement)
        return false;
    let found = false;
    visit(statement, {
...
function functionWritesState(functionNode, stateVars) {
    if (!functionNode?.body)
        return false;
    return statementHasStateWrite(functionNode.body, stateVars);
}
function functionHasAllowedModifier(functionNode, config) {
    const allowed = new Set(config?.accessControl?.modifiers?.length
...
function analyzeFunctionRules(ast, filePath, config, content) {
    const findings = [];
    ...
    visit(ast, {
        FunctionDefinition: (node) => {
            ...
            for (const statement of statements) {
                const hasExternalCall = statementHasExternalCall(statement) ||
...
function collectInternalCalls(body, functions) {
    const calls = [];
    if (!body)
        return calls;
    visit(body, {
...
function statementCallsExternalFunction(statement, functions, summaries) {
    let found = false;
    visit(statement, {
        FunctionCall: (node) => {
            if (node?.expression?.type === 'Identifier') {
...
function analyzeSolidityFile(filePath, config) {
    const findings = [];
    ...
    let ast;
    try {
        ast = (0, solidity_parser_antlr_1.parse)(content, { loc: true, range: true });
    }
    catch {
        return { findings, parseFailed: true };
    }
